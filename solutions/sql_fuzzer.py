...
class SQLFuzzer:
    ...
    def _string_payload_combos(self) -> Iterator[tuple[Optional[str], ...]]:  # noqa: C901
        ...
        attack_values: list[Optional[str]] = list(CORE_PAYLOADS.values())
        for _ in range(5):
            base = self.random.choice(CORE_PAYLOADS.values())
            prefix = self.random.choice(CONTEXT_PREFIXES)
            suffix = self.random.choice(SUFFIXES)
            fuzz = f"{prefix}{base}{self.random.randint(0, 999)}'{suffix}"
