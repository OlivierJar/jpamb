...
```bash
uv run python my_interpreter.py \
    "jpamb.cases.Simple.assertFalse:()V" "()"
```
### Example 2: safeString 

```bash
uv run my_interpreter.py "jpamb.cases.StringSQL.safeString:(Ljava/lang/String;)V" '("hello
")' --verbose 
```
...
```bash
uv run jpamb interpret -W solutions/interpreters/my_interpreter.py
```

### Example 4: run fuzzer 

```bash
uv run jpamb test -W solutions/my_analyzer.py
```
